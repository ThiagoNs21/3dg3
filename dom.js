...
function __static(func,index=undefined){
    ...
    let all = getAll("*",[...methods],index);
    let $ = (arg)=>getAll(arg,[...methods],index); 

    if(ref.indexOf("(")==0){
        ref = ref.substring(1,ref.indexOf(")=>")).split(",").map(f=>{

            if(!f.includes("$")&&f!="all"){
            if(document.querySelector(f.replaceAll("_","-"))!=undefined)
                return getAll(f.replaceAll("_","-"),[...methods],index)
            if(document.querySelector(`.${f.replaceAll("_","-")}`)!=undefined)
                return getAll(`.${f.replaceAll("_","-")}`,[...methods],index)
            if(document.querySelector(`#${f.replaceAll("_","-")}`)!=undefined)
                return getAll(`#${f.replaceAll("_","-")}`,[...methods],index)
...
function __dynamic(func,index=undefined){
    ...
    if(ref.indexOf("(")==0){
        ref = ref.substring(1,ref.indexOf(")=>")).split(",").map(f=>{
            ...
            if(!f.includes("$")){
                if(document.querySelector(f)!=undefined)
...
function getAll(arg,methods,index=undefined){
    ...
    return new class {
        ...
        constructor(){
            ...
            this.addInSelf = function(text){

                let self = text.substring(text.indexOf("{")+1,text.indexOf("}"));
                all.forEach(all=>{
                    let node = text.replaceAll(`{${self}}`,all.outerHTML);
                    all.parentNode.innerHTML = all.parentNode.innerHTML.replaceAll(all.outerHTML,node)
                })

            }
            this.value = standard.property("value");
...
function __transformName(t){
        
    let text = t;
    function replaces(a){

        if(a.includes("-")){
            let upper = a.substring(a.indexOf("-")+1,a.indexOf("-")+2).toUpperCase();
            let ref = a.substring(a.indexOf("-"),a.indexOf("-")+2)
            text = a.replace(ref,upper);
            replaces(text)
        }
    }
...
function __change(func){
    let documentLength = document.querySelectorAll("html")[0].outerHTML;
    setInterval(()=>{
        let gear = true;
        const time = setInterval(()=>{
            if(gear){
                if(documentLength != document.querySelectorAll("html")[0].outerHTML){
                    documentLength = document.querySelectorAll("html")[0].outerHTML;
                    func()
                }       
                gear = false;
            }else{
    ...
    function __openObject(obj,func){
        for(let int in obj){
            func(int,obj[int])
        }
    };
    ...
    function $tag(tag,proper,text=""){
        let attributes = "";
        if(typeof proper == "object"){
            for(let int in proper){
                attributes+=` ${int}="${proper[int]}"`
            }; 
        return `<${tag}${attributes}>${text}</${tag}>`
        }else{
            return `<${tag}>${proper}</${tag}>`  
        }
    }
    function $simpleTag(tag,proper){
        let attributes = "";
            for(let int in proper){
                attributes+=` ${int}="${proper[int]}"`
            }; 
